...
本ドキュメントは、**メールテンプレート専用**のデザインシステム仕様書です。メールクライアント互換性のため、インラインスタイルを使用したブランドカラー、システムカラー、フォント、コンポーネントスタイルの統一されたガイドラインを提供します。

**重要**: このドキュメントはメールテンプレート用に特化しています。**ダッシュボード（管理画面）用のUIライブラリは `ui-library.md` を参照してください。**
...
**用途の明確化**
- ✅ **メールテンプレート**: このドキュメントのインラインスタイルを使用
- ❌ **ダッシュボード（管理画面）**: `ui-library.md` のTailwind CSSコンポーネントを使用
...
- **2026-01-19**: 運用原則準拠のためファイル移動・修正
  - `docs/design-system.md` → `docs/dev/design-system.md` に移動（運用原則: docs/ 直下は INDEX.md のみ）
  - タイトルを「メールテンプレート用デザインシステム」に変更
  - 概要にメールテンプレート専用であることを明記
  - ダッシュボード用UIは `ui-library.md` を参照する注意書きを追加
  - ファイル参照セクションに `ui-library.md` へのリンクを追加
...
### 関連ドキュメント
- ダッシュボード用UIライブラリ: `docs/dev/ui-library.md`
- プロジェクト概要: `CLAUDE.md`
